...
let PRODS=[],ETAT=[],STOCKY=[],TRANSFERTS=[],RECEPTIONS=[],PREVISION=[],PROMOS=[],BUDGET=[],PRIX_MAP={},PRIX_ID_MAP={},DELAIS_MAP={},FORECAST=[];
let COMMENTS_MAP={};
let VN1_NORM={};
function normKey(s){return String(s||'').toLowerCase().replace(/[^a-z0-9]/g,' ').replace(/\s+/g,' ').trim();}
let FOURNISSEURS=[],ABC_MAP={},ABC_ID_MAP={},VN1_MAP={};
let PF='all',CV='alertes',SEL_BUDGET=null;
let PO_EXTRAS={}; // { fournisseur: [{idVariante, nom, variante, quantite}] } — ajouts manuels depuis la bulle ruptures/critiques
let PO_CUSTOM={}; // { fournisseur: [{id, nom, prix, quantite}] } — produits 100% personnalisés (hors catalogue)
let PRIX_OVERRIDE={}; // { 'fournisseur||nom': prixManuel } — surcharge manuelle du prix unitaire
// Table de secours : prix par ID variante Shopify, pour les produits absents de "Prix produits"
// (sous-ensemble fourni manuellement le 2026-07-09 — à étendre au besoin)
...
let PO_ENVOYES={}; // { fournisseur: [{poNumber, lignes:[{idVariante,quantite}], date}] } — commandes déjà créées, pour permettre d'en ajouter/modifier après coup
let MODIF_PO_LINES=[]; // lignes du PO en cours d'édition dans le modal dédié
let MODIF_PO_CTX=null; // {fourn, poNumber}
let SORTS={a:{col:'stock',dir:1},s:{col:'stock',dir:1},f:{col:'pareto',dir:1},v:{col:'vt',dir:-1},e:{col:'nom',dir:1},fc:{col:'nom',dir:1},pr:{col:'nom',dir:1},b:{col:'sn',dir:1}};
let PRODS_A=[],PRODS_S=[];
let VENDOR_MAP={};
let EQUIPE_FILTER='';
function equipeMatch(fourn){
  if(!EQUIPE_FILTER)return true;
...
function setEquipe(eq,el){
  ...
  // Recalculer le budget pour respecter le nouveau filtre équipe
  BUDGET=[];
  for(let i=1;i<=52;i++){
...
function n(v){if(v===''||v===null||v===undefined)return 0;const x=parseFloat(String(v).replace(/[$,\s]/g,''));return isNaN(x)?0:x;}
function fmt(v){return Math.round(n(v)).toLocaleString('fr-CA');}
function fmtM(v){return n(v).toLocaleString('fr-CA',{minimumFractionDigits:0,maximumFractionDigits:0})+' $';}
function fmtD(iso){if(!iso||iso==='')return'—';try{let d;if(typeof iso==='string'&&iso.includes('T')){const p=iso.substring(0,10).split('-');d=new Date(parseInt(p[0]),parseInt(p[1])-1,parseInt(p[2]));}else{d=new Date(iso);}if(isNaN(d))return String(iso).substring(0,10);const dd=String(d.getDate()).padStart(2,'0');const mm=String(d.getMonth()+1).padStart(2,'0');const yy=String(d.getFullYear()).substring(2);return dd+'/'+mm+'/'+yy;}catch{return String(iso).substring(0,10);}}
function cw(){const d=new Date(),s=new Date(d.getFullYear(),0,1);return Math.ceil(((d-s)/86400000+s.getDay()+1)/7);}
function setMsg(m){document.getElementById('lmsg').textContent=m;}
function bP(p){return`<span class="bx ${p==='A'?'ba':p==='B'?'bb':'bc'}">${p||'C'}</span>`;}
function bS(s,sp){
  const sl=String(s||'').toLowerCase();
  if(sl.includes('rupture'))return`<span class="bx br2">Rupture</span>`;
  if(sl.includes('critique')||sl.includes('faible'))return`<span class="bx bam">Critique</span>`;
  return`<span class="bx bgr">Actif</span>`;
}
function sc(v){return v<0?'sn':v<=5?'sl':'';}
...
async function loadData(){
  ...
  try{
    setMsg('Chargement des données en live…');
    ...
    // Tableau correspondance Vendor → Équipe
    VENDOR_MAP={};
    ...
    PRIX_MAP={};
    PRIX_ID_MAP={};
    (raw['Prix produits']||[]).slice(1).forEach(r=>{
      const t=String(r[1]||'').trim();
      const idPp=String(r[3]||'').trim(); // colonne D = ID variante
      const p=n(r[7]);
      if(p>0){
        if(idPp)PRIX_ID_MAP[idPp]=p;
        if(t)PRIX_MAP[normKey(t)]=p;
      }
    });
    ...
    vn1rows.slice(1).forEach(r=>{
      ...
      const id=String(r[3]||'').trim();      // ID variante (col D)
      if(!titre||titre.startsWith('Dernière')||titre.startsWith('Titre'))return;
      let total=0;
      for(let i=4;i<=15;i++){total+=n(r[i]||0);}
      if(total===0)return;
      // Agréger par ID (priorité) pour gérer les renommages de produits
      if(id)VN1_ID_MAP[id]=(VN1_ID_MAP[id]||0)+total;
    ...
    // Ventes N
    const CW=cw();
    const vMap={};       // indexé par "Titre du produit avec variants"
    const vMapById={};   // indexé par ID variante (col D dans Ventes N)
    pT(raw['Ventes N']||[]).forEach(r=>{
      const p=String(r['Titre du produit avec variants']||r['Produit']||'').trim();
      const id=String(r['ID']||r['ID Variante']||r['Variante ID']||'').trim();
    ...
    sRows.slice(1).forEach(r=>{
      ...
      const idVariante=String(r[4]||'').trim();
      ...
      if(statR==='draft'||statR==='archived')return; // exclus partout dans le dashboard
...
demandeCumulee+=fcMois>0?fcMois/4.33:0;      }
      let statut;
      // Rupture ET Critique : seulement si demande > 0 sur le délai
if(stock<0) statut='rupture';
  ...
  en_cmd,fc_m05:fc_cur,wks_left,demande_cumulee:demandeCumulee,vt:vd.total||0,vm:vd.moy||0,vc:vd.curV||0,sems:vd.sems||{},vn1,idVariante,skuFourn});
    ...
    ETAT=pT(raw['Etat des stocks']||[]).filter(r=>r['Nom produit']&&!String(r['Nom produit']).startsWith('Dernière')).map(r=>{
      ...
      const idEtat=String(r['ID']||r['ID Variante']||r['Id produit']||'').trim();
      const variante=String(r['Variante']||'').trim();
      const sems={};for(let i=1;i<=52;i++)sems['S'+String(i).padStart(2,'0')]=n(r['Semaine '+i]);
      return{nom,variante:variante==='Default Title'?'':variante,fourn:String(r['Fournisseur']||''),cat:ABC_ID_MAP[idEtat]||ABC_MAP[nom]||String(r['Catégorie']||'C').trim(),sems};
    });

    // Stocky Orders — parsing positionnel (les en-têtes réels ne correspondent pas
    // aux noms attendus par pT(), donc on se base sur la position des colonnes)
    const byCmd={};
    (raw['Stocky Orders']||[]).slice(1).forEach(r=>{
      const cmd=String(r[5]||'').trim();
      const nomComplet=String(r[1]||'').trim(); // produit + variante déjà combinés
    ...
    // Transferts — même approche positionnelle
    // Colonnes : 0=Clé produit 1=Produit+variante 2=Produit 3=Variante 4=Fournisseur
    //            5=PO 6=Quantité restante 7=Date livraison 8=SKU
    const byCmdT={};
...
livraison:fmtD(String(r[7]||'').trim())||'—',date_cmd:'',lignes:[],total:0};      const qty=n(r[6]||0);
const nouvelleDateT=String(r[9]||'').trim();
const comT=nouvelleDateT?fmtD(nouvelleDateT):'';
byCmdT[cmd].lignes.push({nom:nomComplet,titre:String(r[2]||'').trim(),variante:String(r[3]||''),sku:String(r[8]||'').trim(),qty,livraison:byCmdT[cmd].livraison,com:comT});      byCmdT[cmd].total+=qty;
    });
    TRANSFERTS=Object.values(byCmdT).filter(c=>c.lignes.length>0).sort((a,b)=>b.cmd.localeCompare(a.cmd));

    // Reconstruction de PO_ENVOYES à partir des transferts déjà créés sur Shopify (onglet "Transferts"),
    // pour que le bouton "✎ Modifier" fonctionne même après un rafraîchissement / le lendemain, pas seulement
    // pendant la session où le PO a été créé.
    // ATTENTION : la colonne "Quantité restante" exclut ce qui a déjà été réceptionné. Si une réception
    // partielle a eu lieu sur un PO avant que tu le modifies, la quantité affichée ici sera inférieure à la
    // commande d'origine (elle reflète ce qu'il reste à recevoir, pas ce qui a été commandé au total).
    {
      ...
      // Fusion avec ce qui a été créé pendant la session en cours mais pas encore remonté dans le Sheet
      // (le script de synchronisation Transferts tourne périodiquement, pas en temps réel après un doPost).
    ...
    // Prévision commandes (Nina + Clovis fusionnées — le filtre équipe s'occupe de la séparation à l'affichage)
    PREVISION=pT(raw['Prevision commandes']||[]).filter(r=>{
      ...
      const prixParId=idPrev?(PRIX_ID_MAP[idPrev]||0):0;
      const prixParNom=PRIX_MAP[normKey(nom)]||(pMatch?PRIX_MAP[normKey(pMatch.nb)]||0:0)||0;
      const prixFinal=prixParId>0?prixParId:(prixParNom>0?prixParNom:(idPrev?(PRIX_FALLBACK_ID[idPrev]||0):0));
      return{nom,fourn:String(r['Fournisseur']||''),cat:(idPrev&&ABC_ID_MAP[idPrev])||ABC_MAP[nom]||String(r['Catégorie']||'C'),
        ...
        prix:prixFinal,vm:vdp.moy||0,sems,idVariante:idPrev};
    });
    ...
    // Promos
    // Promos: colonne J = prix promo (lu par nom de header via pT)
PROMOS=pT(raw['Promos']||[]).filter(r=>r['Produit']&&(r['Date Début']||r['Date Fin'])).map(r=>{
  const boost=n(String(r['Boost%']||'0').replace('%',''));
    ...
    // Budget (respecte le filtre équipe : Nina ne voit jamais le budget de Clovis, et vice versa)
    BUDGET=[];
    for(let i=1;i<=52;i++){
      const val=PREVISION.reduce((s,r)=>equipeMatch(r.fourn)?s+(r.sems[i]||0)*(r.prix||0):s,0);
      BUDGET.push({label:'Semaine '+i,val:Math.round(val),sn:i});
...
function populateFiltresFc(){
  ...
  if(!ff||ff.options.length>1)return;
...
function populateFiltres(){
  ...
  ['f-a','f-s','f-v','f-r'].forEach(id=>{const el=document.getElementById(id);if(el)el.innerHTML=opts;});
  const sf=document.getElementById('s-f');
  const fournsFiltered=FOURNISSEURS.filter(f=>equipeMatch(f));
  ...
  if(ddlSwpo&&!ddlSwpo.childElementCount){
    ddlSwpo.innerHTML=(()=>{const arr=[];for(let i=Math.max(1,W-4);i<=Math.min(52,W+12);i++)arr.push(i);return arr;})()
...
function renderView(v){
  ...
  else if(v==='po')rPO();else if(v==='budget')rBudget();else if(v==='promos')rPromos();
  else if(v==='forecast')rForecast();
}

function srt(tbl,col,el){
  const s=SORTS[tbl];
  const viewMap={a:'alertes',s:'stocks',f:'fournisseurs',v:'ventes',fc:'forecast',pr:'promos',b:'budget'};
  ...
  if(tbl==='a')rAlertes();else if(tbl==='s')rStocks();else if(tbl==='f')rFourn();else if(tbl==='v')rVentes();else if(tbl==='fc')rForecast();else if(tbl==='pr')rPromos();else if(tbl==='b')rBudget();
}
function sortProds(arr,col,dir){
  return [...arr].sort((a,b)=>{
    let va,vb;
    if(col==='solde'){va=a.stock+a.en_cmd;vb=b.stock+b.en_cmd;}
    else if(col==='growth'){va=a._growth;vb=b._growth;}
    else if(col==='total'){va=a._total;vb=b._total;}
    else if(typeof col==='string'&&col.indexOf('wk_')===0){const wk=col.slice(3);va=(a.sems&&a.sems[wk])||0;vb=(b.sems&&b.sems[wk])||0;}
...
if(p.statut!=='rupture'&&p.demande_cumulee<=0)return false;    if(!equipeMatch(p.fourn))return false;
    ...
    if(srch&&!p.nom.toLowerCase().includes(srch))return false;
    return true;
  });
...
function rStocks(){
  ...
  document.getElementById('tb-s').innerHTML=rows.map(p=>`<tr>
    ...
    <td>${bS(p.statut,p.statut_produit)}</td>
    <td style="text-align:right;font-size:12px">${p.wks_left!==null?p.wks_left+' sem.':'—'}</td>
<td style="text-align:right">${p.en_cmd>0?`<span style="cursor:pointer;color:var(--bl);text-decoration:underline;font-weight:500" onclick="navReceptions(this)" data-nom="${p.nom.replace(/"/g,'&quot;')}" title="Voir dans Réceptions">${fmt(p.en_cmd)}</span>`:'—'}</td>    <td style="text-align:right">${fmt(p.fc_m05)}</td>
    <td style="text-align:right;font-weight:500;color:${p.statut==='rupture'?'var(--re)':p.statut==='critique'?'var(--am)':'var(--gr)'}">${fmt(p.stock+p.en_cmd)}</td>
  </tr>`).join('')||'<tr><td colspan="9" style="text-align:center;padding:40px;color:var(--t3)">Aucun résultat</td></tr>';
}

function rFourn(){
  const sel=document.getElementById('s-f')?.value||'';
  const pars=gC('pf'),stats=gC('stf');
  const fc=document.getElementById('fc');
  if(!sel){fc.innerHTML='<p style="color:var(--t3);padding:20px">Sélectionnez un fournisseur</p>';return;}
  ...
  fc.innerHTML=`
    ...
    <div class="tw"><table>
      <thead><tr><th onclick="srt('f','nom',this)">Produit</th><th onclick="srt('f','pareto',this)" class="asc">Pareto</th><th style="text-align:right" onclick="srt('f','stock',this)">Stock</th><th onclick="srt('f','statut',this)">Statut</th><th style="text-align:right" onclick="srt('f','en_cmd',this)">En commande</th><th style="text-align:right" onclick="srt('f','fc_m05',this)">Forecast M05</th></tr></thead>
      <tbody>${sorted.map(p=>`<tr>
        <td><div class="pn">${p.nom}</div>${p.variante?`<div class="pv">${p.variante}</div>`:''}</td>
        <td>${bP(p.pareto)}</td><td style="text-align:right"><span class="${sc(p.stock)}">${fmt(p.stock)}</span></td>
        <td>${bS(p.statut,p.statut_produit)}</td><td style="text-align:right">${p.en_cmd>0?fmt(p.en_cmd):'—'}</td>
        <td style="text-align:right">${fmt(p.fc_m05)}</td>
      </tr>`).join('')}</tbody>
    </table></div>`;
}
...
function rVentes(){
  ...
  const rowsFiltered=PRODS.filter(p=>{
    ...
    if(pars.length&&!pars.includes(p.pareto))return false;
    if(srch&&!p.nom.toLowerCase().includes(srch))return false;
    return p.vt>0||p.vn1>0;
  });
  rowsFiltered.forEach(p=>{
    p._growth=null;
    if(p.vn1>0){
      const n1ytd=Math.round(p.vn1*(W/52));
      if(n1ytd>0)p._growth=(p.vt-n1ytd)/n1ytd;
    }
  });
  ...
  document.getElementById('th-v').innerHTML=`<tr>
    ...
    ${dispWks.map(k=>`<th style="text-align:right" onclick="srt('v','wk_${k}',this)">${k}</th>`).join('')}
    <th style="text-align:right" onclick="srt('v','vt',this)">Total N</th>
    <th style="text-align:right" onclick="srt('v','vn1',this)">Total N-1</th>
    <th style="text-align:right" onclick="srt('v','growth',this)">Croissance</th>
...
function rReceptions(autoOpen){
  const srch=(document.getElementById('s-r')?.value||'').toLowerCase();
  const fourn=document.getElementById('f-r')?.value||'';
  const par=document.getElementById('p-r')?.value||'';
  const stat=document.getElementById('st-r')?.value||'';
  const sw=document.getElementById('sw-r')?.value||'';
  const W=cw();

  function matchSemaine(c){
    if(!sw)return true;
    if(!c.livraison||c.livraison==='—')return true; // pas de date connue : ne pas exclure
    try{
...
function navReceptions(el){
  const nom=typeof el==='string'?el:(el.dataset?.nom||'');  document.querySelectorAll('.ni').forEach(e=>e.classList.remove('on'));
  document.querySelector('.ni[onclick*="receptions"]')?.classList.add('on');
  document.querySelectorAll('.view').forEach(e=>e.classList.remove('on'));
  document.getElementById('v-receptions').classList.add('on');
  CV='receptions';
...
function rPO(){
  ...
  Object.entries(PO_EXTRAS).forEach(([f,lignes])=>{
    ...
    lignes.forEach(l=>{
      ...
      const prixExtra=(PRIX_ID_MAP[l.idVariante]||0)||PRIX_MAP[normKey(l.nom)]||(PRIX_FALLBACK_ID[l.idVariante]||0);
      rowsExtra.push({nom:l.nom,fourn:f,cat:p?p.pareto:'C',delai:DELAIS_MAP[f]||0,
  ...
  function idVEnvoyesFor(f){return new Set((PO_ENVOYES[f]||[]).flatMap(e=>e.lignes.map(l=>l.idVariante)));}
  function qtyEnvoyeeFor(f,idV){return (PO_ENVOYES[f]||[]).reduce((s,e)=>s+e.lignes.filter(l=>l.idVariante===idV).reduce((s2,l)=>s2+(l.quantite||0),0),0);}
  ...
  function qtySel(r){return semaines.reduce((s,sw)=>s+(r.sems[sw]||0),0);}
...
function majQuantitePO(fourn,idVariante,nom,sw,val,tipo,customId){
  const qte=Math.max(0,parseInt(val)||0);
  if(tipo==='manuel'){
    majQuantiteExtra(fourn,idVariante,qte);
  } else if(tipo==='custom'){
    const cu=(PO_CUSTOM[fourn]||[]).find(x=>x.id===customId);
    if(cu)cu.quantite=qte;
    rPO();
  } else {
    const r=PREVISION.find(x=>x.fourn===fourn&&x.nom===nom);
    if(r)r.sems[sw]=qte;
  }
  rPO();
}
function majPrixPO(fourn,nom,val){
  ...
  if(v>0)PRIX_OVERRIDE[k]=v;else delete PRIX_OVERRIDE[k];
  rPO();
}
function ajouterProduitPersonnalise(fourn,blocId){
  const nomEl=document.getElementById('cust-nom-'+blocId);
  const prixEl=document.getElementById('cust-prix-'+blocId);
  const qteEl=document.getElementById('cust-qte-'+blocId);
  ...
  const id='custom-'+Date.now()+'-'+Math.round(Math.random()*100000);
  PO_CUSTOM[fourn].push({id,nom,prix,quantite:qte});
  rPO();
}
function retirerCustom(fourn,id){
  if(!PO_CUSTOM[fourn])return;
  PO_CUSTOM[fourn]=PO_CUSTOM[fourn].filter(x=>x.id!==id);
  rPO();
}
function ajouterHorsPO(idVariante){
  const p=PRODS.find(x=>x.idVariante===idVariante);
  if(!p)return;
  const qtyInput=document.getElementById('qty-hp-'+idVariante);
  const qte=Math.max(1,parseInt(qtyInput?.value)||1);
  if(!PO_EXTRAS[p.fourn])PO_EXTRAS[p.fourn]=[];
  if(PO_EXTRAS[p.fourn].some(l=>l.idVariante===idVariante))return;
  PO_EXTRAS[p.fourn].push({idVariante:p.idVariante,nom:p.nom,variante:p.variante||'',quantite:qte});
  rPO();
}
function retirerExtra(fourn,idVariante){
  if(!PO_EXTRAS[fourn])return;
  PO_EXTRAS[fourn]=PO_EXTRAS[fourn].filter(l=>l.idVariante!==idVariante);
  rPO();
}
function majQuantiteExtra(fourn,idVariante,val){
  const l=(PO_EXTRAS[fourn]||[]).find(x=>x.idVariante===idVariante);
  if(l)l.quantite=Math.max(0,parseInt(val)||0);
}

function rechercherProduitBlock(fourn,blocId){
  const q=document.getElementById('search-'+blocId).value.trim().toLowerCase();
  const resDiv=document.getElementById('search-res-'+blocId);
  if(!q){resDiv.style.display='none';resDiv.innerHTML='';return;}

  const dejaDansExtras=new Set((PO_EXTRAS[fourn]||[]).map(l=>l.idVariante));
  const matches=PRODS.filter(p=>
    p.idVariante && p.fourn===fourn && !dejaDansExtras.has(p.idVariante) && p.nom.toLowerCase().includes(q)
  ).slice(0,8);
  ...
  resDiv.innerHTML=matches.map(p=>{
    const idSafe=p.idVariante.replace(/'/g,"\\'");
    return `<div style="padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--b1);font-size:12px" onclick="ajouterProduitBlock('${fourn.replace(/'/g,"\\'")}','${idSafe}')">
...
function ajouterProduitBlock(fourn,idVariante){
  ...
  if(PO_EXTRAS[fourn].some(l=>l.idVariante===idVariante))return;
  PO_EXTRAS[fourn].push({idVariante:p.idVariante,nom:p.nom,variante:p.variante||'',quantite:1});
  rPO();
}

// ============================================================
// Génération PO fournisseur — document imprimable (style Shopify)
// ============================================================
// lignes attendues : [{nom, variante, sku, qte, prix}]
function ouvrirDocumentPO(fourn, poNum, dateLivraison, lignes){
  ...
  if(!w){ alert("Le navigateur a bloqué l'ouverture de la fenêtre. Autorise les pop-ups pour ce site."); return; }
...
function genererPDFPO(idx, semaines){
  ...
  if(!dejaEnvoyes.length){ alert('Aucune commande créée pour ce fournisseur — crée le PO d\'abord.'); return; }
  ...
  // Cumul des quantités réellement envoyées (commande initiale + compléments), par idVariante
  const totalParId={};
  ...
  const lignes = Object.entries(totalParId).filter(([,qte])=>qte>0).map(([idV,qte])=>{
    ...
    const prix = (p && PRIX_OVERRIDE[kOv]!=null) ? PRIX_OVERRIDE[kOv] : prixLigneManuelle({idVariante:idV,nom:p?p.nom:''});
    return{
...
// ============================================================
// Modification d'un PO déjà envoyé — modal dédié, isolé du tableau
// principal : on choisit un PO précis (ex: "#T0037") et seules SES
// lignes s'affichent, sans mélange avec d'autres PO ou de nouveaux
// produits en attente.
// Une augmentation de quantité crée automatiquement un complément
// Shopify (nouveau transfert, même logique que "Ajouter X produit(s)").
// Une diminution n'est pas envoyée à Shopify automatiquement (pas
// de mutation confirmée pour retirer des lignes d'un transfert
// existant) — on avertit et on ne modifie rien pour cette ligne.
// ============================================================
// Télécharge le PDF d'un seul PO précis (pas le cumul de tous les PO du fournisseur,
// contrairement au bouton global "📄 PDF fournisseur").
function telechargerPDFUnPO(fourn, poNumber){
  ...
  const lignes = entree.lignes.filter(l=>l.quantite>0).map(l=>{
    ...
    return {
      ...
      qte: l.quantite,
      prix: p ? (PRIX_ID_MAP[l.idVariante]||PRIX_MAP[normKey(p.nom)]||PRIX_FALLBACK_ID[l.idVariante]||0) : 0
    };
  });
...
// Recherche pour ajouter un produit qui ne fait pas encore partie de ce PO
// (soit un produit jamais commandé pour ce fournisseur, soit un produit dont
// la ligne n'a pas pu être rattachée automatiquement au catalogue).
...
function ajouterProduitModifPO(idVariante){
  ...
  MODIF_PO_LINES.push({
    ...
    prix:PRIX_ID_MAP[idVariante]||PRIX_MAP[normKey(p.nom)]||PRIX_FALLBACK_ID[idVariante]||0
  });
  document.getElementById('mp-add-input').value='';
  document.getElementById('mp-add-res').style.display='none';
  renderLignesModifPO();
...
// ============================================================
let MANUAL_LINES = []; // [{idVariante, nom, variante, quantite}]
let DERNIERE_COMMANDE_MANUELLE = null; // {fourn, poNumber, dateLivraison, lignes}
...
function prixLigneManuelle(l){
  return (PRIX_ID_MAP[l.idVariante]||0)||PRIX_MAP[normKey(l.nom)]||(PRIX_FALLBACK_ID[l.idVariante]||0);
}
...
function rBudget(){
  const W=cw();
  const rows=sortProds(BUDGET,SORTS.b.col,SORTS.b.dir);
  document.getElementById('tb-b').innerHTML=rows.map(b=>`
...
function toggleBudget(sn,label,total){
  ...
  if(!det)return;
  if(SEL_BUDGET===sn){SEL_BUDGET=null;det.style.display='none';rBudget();return;}
  SEL_BUDGET=sn;
  const byF={};
  PREVISION.forEach(r=>{
    const qty=r.sems[sn]||0;
    if(!qty||!r.fourn)return;
...
function navPO(el){const fourn=el?el.dataset.fourn.replace(/&#39;/g,"'"):arguments[0];const sw=el?+el.dataset.sw:arguments[1];
...
function setPF(f,el){PF=f;document.querySelectorAll('#v-promos .fb').forEach(b=>b.classList.remove('on'));el.classList.add('on');rPromos();}
function rPromos(){
  ...
  const rows=PROMOS.filter(r=>{
    ...
    if(srch&&!r.produit.toLowerCase().includes(srch))return false;
    return true;
  });
  ...
  document.getElementById('tb-pr').innerHTML=rows.map(r=>{
    ...
    const prixHtml=r.prixPromo>0
      ...
      :'—';
    return`<tr>
<td><div class="pn">${r.produit}</div>${(()=>{const p=PRODS.find(x=>x.nom===r.produit||x.nb===r.produit);return p&&p.variante?`<div class="pv">${p.variante}</div>`:r.variante?`<div class="pv">${r.variante}</div>`:''})()}</td>      <td style="white-space:nowrap">${r.marque}</td>
      <td>${badge}</td>
      ...
      <td style="text-align:right;font-size:11px;color:var(--t3)">${r.boost>0?r.boost+'%':'—'}</td>
...
// DROPDOWN CHECKBOX
function gC(name){return[...document.querySelectorAll('input[name="'+name+'"]:checked')].map(e=>e.value);}
function sC(name,vals){document.querySelectorAll('input[name="'+name+'"]').forEach(e=>{e.checked=vals.includes(e.value);});}
...
function populateFournDD(){
  [['ddl-fa','fa','rAlertes','dd-fa'],['ddl-fs','fs','rStocks','dd-fs'],
   ...
   ['ddl-ffc','ffc','rForecast','dd-ffc']].forEach(([lid,name,cb,did])=>{
...
function closeSidebarOnNav(){
  if(window.innerWidth<=768){document.querySelector('.sidebar').classList.remove('open');document.getElementById('overlay').classList.remove('open');}
}
